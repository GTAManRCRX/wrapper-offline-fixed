					...
					const req = https.request({
						...
						res.on("end", () => {
							try {
								...
								if (json.success !== true) {
									return rej(`Polly proxy error: ${json.error_msg || "Unknown error"}`);
								}
								https.get(json.audio_url, (audioRes) => {
									if (audioRes.statusCode !== 200) {
										return rej(`Polly audio download error: ${audioRes.statusCode}`);
									}
									resolve(audioRes);
								}).on("error", rej);
							} catch (e) {
								rej("Polly proxy error: Invalid JSON response from lazypy");
							}
						});
					});
					req.on("error", (e) => rej(`Network error: ${e.message}`));
					req.end(body);
					break;
				}
